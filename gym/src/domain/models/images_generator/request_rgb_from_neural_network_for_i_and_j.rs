...
>(
    ...
    let distance_from_top = (manhattan_distance(i, j, image_width - 1  / 2, 0) as f64) / (image_height as f64);
    let distance_from_top_left = (manhattan_distance(i, j, 0, 0) as f64) / (image_height as f64) ;
    let distance_from_left = (manhattan_distance(i, j, 0, image_height -1  / 2) as f64) / (image_height as f64);
    let distance_from_center = (manhattan_distance(i, j, image_width - 1 / 2, image_height - 1 / 2) as f64) / (image_height as f64);
    let distance_from_right = (manhattan_distance(i, j, image_width - 1, image_height - 1 / 2) as f64) / (image_height as f64);
    let distance_from_bottom = (manhattan_distance(i, j, image_width -1  / 2, image_height - 1) as f64) / (image_height as f64);
    let distance_from_bottom_right = (manhattan_distance(i, j, image_height - 1, image_width - 1) as f64) / (image_height as f64);
    ...
    assert!(distance_from_bottom_right <= 2.0, "{}", distance_from_bottom_right);
